...
<?php
			            foreach ( $top_links as $link ) {
			            	printf( '<li class=%3$s><a href="%1$s">%2$s</a></li>', $link['url'], $link['name'], $link['class'] );
			            }
?>
			        </ul>
			    </nav>
...
<?php
		    foreach ( $dir as $d ) {
		        ...
		        foreach( glob( $d ) as $file )  {
		            ...
		            echo '<li>';
		            if (in_array($project, $playsites)) {
		            	$siteroot = sprintf( 'http://%1$s.%2$s:9000', $project, $tld );
		            } else if (in_array($project, $embersites)){
		            	$siteroot = sprintf( 'http://%1$s.%2$s:4200', $project, $tld );
		            } else {
...
?>
    ...
    <script>
    $('.sites li').on('click', function (e){
    	var $clickedLI = $(e.target);
    	window.location = $clickedLI.children('.site').get(0).href;
    });
    </script>
</html>
